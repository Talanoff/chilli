...
class CatalogController extends BaseController
{
	...
	protected function getFile()
	{
		...
		if (empty($fileName)) {
			return $this->failure('Mode: ' . $this->stepFile . ', parameter filename is empty');
		}
		...
		if (empty($fData)) {
			return $this->failure('Mode: ' . $this->stepFile . ', input data is empty.');
		}
...
\Log::info($path . '/' . $workDirName . '/' . $fileName);
	...
	protected function checkInputPath()
	{
		...
		if (empty($folderName)) {
			$folderName = date('Y-m-d_H:i') . '_' . time();
